...
namespace {
    ...
    /* Available shifts. */
    const HashSet<Shift> kStandardShifts = {
        { Day::SUNDAY,  8, 14, 0 },
        { Day::SUNDAY, 12, 18, 0 },
...
void ShiftSchedulingGUI::actionPerformed(GObservable* source) {
    if (source == mImpl->mSolve) {
        ...
        mImpl->mChosen = setCast<Set<Shift>>(highestValueScheduleFor(setCast<HashSet<Shift>>(mImpl->mShifts), kStandardHours));
        requestRepaint();
...
shared_ptr<ShiftSchedulingGUI> makeShiftSchedulingGUI(GWindow& window) {
