/* File: topK.cpp
 * ---------------
 * This file uses the HeapPQueue class that has been defined to implement the topK function, which takes a stream of DataPoints and an integer k and
 * returns a vector with the k elements with the highest weights, in descending order.
 */
...
/* Function: topK
 * ---------------
 * This function takes a stream of DataPoints and an integer k, and returns a vector with the k elements with the highest weights, in descending order
 */
Vector<DataPoint> topK(istream& stream, int k) {
    ...
    // Decrease the size of the HeapPQueue
    while (storage.size() > k) {
        storage.dequeue();
    }
    // Initialize a vector with the size of the HeapPQueue. This will be k if k<size() or size of the queue if k is larger than number of elements in the queue
    Vector<DataPoint> vec(storage.size());
    for (int i = storage.size() - 1; i >= 0; i--) {
        vec[i] = storage.dequeue();
    }
    return vec;
}
...
ADD_TEST("User test: stream k elements, ask for top k elements") {
    ...
    Vector<DataPoint> expected = { vec[9], vec[8], vec[7], vec[6], vec[5], vec[4], vec[3], vec[2], vec[1], vec[0] };
    EXPECT_EQUAL(topK(stream, vec.size()), expected);
}

ADD_TEST("User test: stream 0 elements, ask for a negative number of elements") {
    auto stream = asStream({});
    Vector<DataPoint> expected = {};
    EXPECT_ERROR(topK(stream, -2));
}
