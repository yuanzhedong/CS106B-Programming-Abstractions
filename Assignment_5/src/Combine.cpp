/* File: Combine.cpp
 * ---------------
 * Combines a list of vectors into a single, sorted vector. Does so by recursively breaking down each vector until only its last element remains,
 * and then combining those vectors such that the combined vector is sorted.
 */
...
/* Function: combine
 * ------------------
 * The function combine is a recursive function, where the base case is that if there is only a single element or less in the vector,
 * it returns the first element in the vector. The recursive case divides the vector into two halves, and then combines those two halves.
 * When combining, it uses the function merge, which is defined below.
 */
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<DataPoint> leftCombine = combine(left);
    Vector<DataPoint> rightCombine = combine(right);
    return merge(leftCombine, rightCombine);
}

/* Function: merge
 * ---------------
 * The function merge takes in two lists of vectors and returns a single sorted vector. It takes advantage of the fact that each vector is already in sorted
 * order by only comparing the next elements in each vector. It uses counters to ensure that the function is efficient, instead of removing elements from the
 * vector, which would cause runtime to be higher given that the vector::remove function would require moving each element of the vector each time it is called
 */
Vector<DataPoint> merge(Vector<DataPoint> left, Vector<DataPoint> right) {
    Vector<DataPoint> mergedVec = {};
    int leftCounter = 0;
    int rightCounter = 0;
    // while the counters are within the size of each vector
    while (leftCounter < left.size() || rightCounter < right.size()) {
        // series of if statements to check if we are at the end of any of the two vectors, and if not, to check which element has a lower weight and add that
        if (leftCounter == left.size()) {
            ...
            leftCounter++;
        } else if (left[leftCounter].weight < right[rightCounter].weight) {
            mergedVec += left[leftCounter];
            leftCounter++;
        } else {
            mergedVec += right[rightCounter];
...
ADD_TEST("User test: Merges two empty lists") {
    Vector<DataPoint> vecOne = {};
    Vector<DataPoint> vecTwo = {};
    auto merged = combine({vecOne, vecTwo});
